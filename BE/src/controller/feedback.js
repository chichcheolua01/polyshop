...
export const getOne = async (req, res) => {
    try {
        const data = await Feedback.findOne({ user: req.params.id }).populate('user');
...
export const update = async (req, res) => {
    try {
        ...
        const decoded = jwt.verify(token, process.env.SECRET_KEY);
        const feed_back = await Feedback.findById(req.params.id)
        console.log(feed_back.user._id);
        if (feed_back.user._id == decoded.id) {
            const feedBack = await Feedback.findById(req.params.id)
            if (!feedBack) {
                return res.status(404).json({ message: "Không có phản hồi" })
            }
            ...
            if (!data) {
                return res.status(404).json({ message: "Cập nhật phản hồi thất bại" });
            }
