...
export const register = async (req, res) => {
    try {
        ...
        if (req.body.password.length < 6) {
            return res.status(404).json({ message: 'Mật khẩu phải từ 6 ký tự trở lên' })
        ...
        return res.status(200).json({ message: 'Đăng ký tài khoản thành công', data: data })
...
export const logIn = async (req, res) => {
    try {
        ...
        const passwordHash = await bcrypt.compare(req.body.password, email.password)
        if (!passwordHash) {
        ...
        return res.status(200).json({ message: 'Đăng nhập tài khoản thành công', data: email, token: token })
